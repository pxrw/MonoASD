...
class Trainer(object):
        ...
        with torch.no_grad():
            for batch_idx, (inputs, calibs, coord_ranges, targets, info) in enumerate(self.train_loader):
                ...
                if self.model_type == 'DID':
                    ...
                    rgb_outputs, backbone_loss_affinity, head_loss, align_select_loss = self.model(inputs, coord_ranges,
                                                                                                 calibs, targets,
                                                                                                 self.epoch)
                    criterion = DIDLoss(self.epoch)
    ...
    def train_one_epoch_distill(self, loss_weights=None):
        ...
        if 'feat_align_kd' in self.kd_type:
            loss_stats.append('feat_align_kd')
        if 'self_select_kd' in self.kd_type:
            loss_stats.append('self_select_kd')
        ...
        for batch_idx, (inputs, calibs, coord_ranges, targets, info) in enumerate(self.train_loader):
            ...
            if 'spearman_kd' in self.kd_type:
                rgb_outputs, relation_loss, backbone_loss_affinity, head_loss, feat_align_loss = self.model(inputs,
                                                                                                            coord_ranges,
                                                                                                            calibs,
                                                                                                            targets)
            else:
            ...
            if 'feat_align_kd' in self.kd_type:
                feat_align_loss = (feat_align_loss * 20).mean()
                if 'feat_align_kd' not in stat_dict.keys():
                    stat_dict['feat_align_kd'] = 0
                stat_dict['feat_align_kd'] += feat_align_loss

                if 'feat_align_kd' not in disp_dict.keys():
                    disp_dict['feat_align_kd'] = 0
                disp_dict['feat_align_kd'] += feat_align_loss

                total_loss = total_loss + feat_align_loss

            if 'self_select_kd' in self.kd_type:
                self_select_loss = (self_select_loss * 10).mean()
                if 'self_select_kd' not in stat_dict.keys():
                    stat_dict['self_select_loss'] = 0
                stat_dict['self_select_loss'] += self_select_loss

                if 'self_select_kd' not in disp_dict.keys():
                    disp_dict['self_select_kd'] = 0
                disp_dict['self_select_kd'] += self_select_loss

                total_loss = total_loss + self_select_loss

            if 'align_select_kd' in self.kd_type:
                align_select_loss = (align_select_loss * 10).mean()
                if 'align_select_kd' not in stat_dict.keys():
                    stat_dict['align_select_kd'] = 0
                stat_dict['align_select_kd'] += align_select_loss

                if 'align_select_kd' not in disp_dict.keys():
                    disp_dict['align_select_kd'] = 0
                disp_dict['align_select_kd'] += align_select_loss

                total_loss = total_loss + align_select_loss
