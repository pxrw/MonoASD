...
# from lib.align.fusion_align import FeatureAlignLoss
# from lib.align.mamba_select_align import SelfSelectLoss
from lib.align.align_select import AlignSelectLoss
...
class DID_Distill(nn.Module):
    def __init__(self, backbone='dla34', neck='DLAUp', downsample=4, flag='training', mean_size=None, model_type='distill', cfg=None):
        ...
        # self.compute_feat_align_loss = FeatureAlignLoss(channels[2:])
        # self.compute_self_select_loss = SelfSelectLoss(channels[2:])
        self.compute_align_select_loss = AlignSelectLoss(channels[2:])
    ...
    def forward(self, input, coord_ranges, calibs, target=None, K=50, mode='train'):
        ...
        if mode == 'train' and target != None:
            ...
            if 'dlaup_kd' not in self.cfg['kd_type']:
                ...
                ### distillation loss
                # feat_align_loss, aligned_stu, aligned_tea = self.compute_feat_align_loss(distill_feature, depth_fs_feat)

                # self_select_loss, selected_stu, selected_tea = self.compute_self_select_loss(aligned_stu, aligned_tea)
                align_select_loss, distill_feature, depth_fs_feat = self.compute_align_select_loss(distill_feature, depth_fs_feat)
                ...
                return rgb_outputs, backbone_loss_affinity, head_loss, align_select_loss
